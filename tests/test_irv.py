#!/usr/bin/env python3
"""
Test script for hypothetical transfers feature in IRV analysis.
...
def test_hypothetical_transfers():
    """Test the hypothetical transfers feature with a simple IRV example."""
...
{title: "Test Hypothetical Transfers"}
Kiss:5
Montroll:4
Wright:3
Kiss:2
...
Wright:6''',
        'include_IRV': 'yes',
        'include_irv_extra': 'yes'  # This enables hypothetical transfers
    }

    print("Testing hypothetical transfers with simple IRV data...")
    print("Input data:")
    ...
    if response.status_code == 200:
        ...
        if 'Hypothetical transfers if eliminated this round:' in html_content:
            print("✓ SUCCESS: Hypothetical transfers feature is working!")

            # Find and display the IRV table section
            import re
            irv_match = re.search(r'<h2[^>]*>IRV.*?</table>', html_content, re.DOTALL | re.IGNORECASE)
            if irv_match:
                print("\nIRV Table with Hypothetical Transfers:")
                print("-" * 50)
            ...
            print("✗ FAILED: Hypothetical transfers not found in output")
